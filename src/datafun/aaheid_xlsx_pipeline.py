...
Date: 2026-04
  ...
  Paths (relative to repo root):
    ...
    OUTPUT FILE: data/processed/xlsx_feedback_data_count.txt
...
def run_xlsx_pipeline(*, raw_dir: Path, processed_dir: Path, logger: Any) -> None:
    ...
    output_file = processed_dir / "xlsx_feedback_data_count.txt"
    ...
    logger.info(
        "XLSX: counted word '%s' in column %s and wrote %s",
        word,
        column_letter,
        output_file,
    )
